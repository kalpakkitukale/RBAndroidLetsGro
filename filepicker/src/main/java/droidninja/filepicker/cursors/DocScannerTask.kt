...
class DocScannerTask(
    ...
    private val resultCallback: FileMapResultCallback?
) : AsyncTask<Void, Void, Map<FileType, List<Document>>>() {
    ...
    private fun createDocumentType(documents: ArrayList<Document>): HashMap<FileType, List<Document>> {
        ...
        for (fileType in fileTypes) {
            val documentListFilteredByType =
                documents.filter { document -> document.isThisType(fileType.extensions) }
    ...
    override fun doInBackground(vararg voids: Void): Map<FileType, List<Document>> {
        ...
        Log.d("StudentClassroomTag", "selection------ $selection")
        ...
        if (cursor != null) {
            Log.d("StudentClassroomTag", "Cursor Count ------ ${cursor.count}")
...
//            Log.d("StudentClassroomTag", "File Path ----- $path")

            if (path != null) {

                val fileType = getFileType(PickerManager.getFileTypes(), path)
//                Log.d("StudentClassroomTag", "File Type ----- $fileType")
                val file = File(path)
